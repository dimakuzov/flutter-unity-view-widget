...
namespace ARLocation
{
    ...
    public class ARLocationConfigInspector : Editor
    {
        ...
        const string ARGPS_USE_ARDK = "ARGPS_USE_ARDK";
        ...
        private void OnEnable()
        {
            ...
            p_UseARDK = serializedObject.FindProperty("UseARDK");
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.PropertyField(p_UseARDK);
		...
		EditorGUILayout.HelpBox("Make sure that Vuforia is instaled in the Package Manager Window.  On Android, also make sure that the 'ARCore XR Plugin' is not installed.", MessageType.Info);
            ...
            if (p_UseARDK.boolValue)
            {
#if UNITY_2019_3_OR_NEWER
                EditorGUILayout.HelpBox("Make sure that ARDK is installed in the Package Manager Window. ", MessageType.Info);
#endif
            }
            ...
            UpdateDefineSymbolPropConfig(config.UseARDK, p_UseARDK.boolValue, ARGPS_USE_ARDK);
            
             UpdateARDKPlayerSettings(config.UseARDK, p_UseARDK.boolValue);

            if (!config.UseARDK)
            {
                // Make sure only one SDK is active.
        ...
        private void UpdateARDKPlayerSettings(bool oldValue, bool newValue)
        {
            ...
            if (newValue)
            {
                PlayerSettings.ardkEnabled = true;
            }
            else
            {
                PlayerSettings.ardkEnabled = false;
            }
#else
